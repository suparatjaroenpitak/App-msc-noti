/**
 * Built-in analysis engine ("builtin-v1") — ported from the server.
 * Deterministic indicators (SMA, RSI, volatility, momentum) with a
 * conservative verdict; runs fully offline on the device.
 */
...
export function analyze(input: EngineInput): EngineOutput {
  ...
  // Momentum vs the first half-average of available samples.
  ...
  // Position within the observed price range (0 = low, 100 = high).
  ...
  // ---------- Suggested levels ----------
  const atrLike = Math.max(input.currentPrice * (vol / 100), input.currentPrice * 0.005);
  const entry = round2(input.currentPrice - atrLike * 0.5);
  const stop = round2(entry - atrLike * 1.5);
  const target = round2(entry + atrLike * 2.5);

  const dataScore = Math.min(1, all.length / 40);
